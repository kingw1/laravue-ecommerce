<template>
    <header class="flex justify-between items-center h-12 shadow bg-white">
        <button @click="emit('toggle-sidebar')" class="p-4 text-gray-700 hover:bg-black/10 transition-colors">
            <MenuIcon class="w-6" />
        </button>
        <Menu as="div" class="relative inline-block text-left px-4">
            <div>
                <MenuButton class="flex items-center">
                    <img src="https://randomuser.me/api/portraits/men/84.jpg" class="rounded-full w-8 mr-2">
                    <small>King Doe</small>
                    <ChevronDownIcon class="ml-2 h-5 w-5 text-indigo-200 hover:text-indigo-100" aria-hidden="true" />
                </MenuButton>
            </div>
